...
mod tests {
    ...
    fn test_rrf_vs_weighted_averaging() {
        ...
        for (doc, r_vec, r_bm25, r_fuzzy) in all_docs {
            let mut bonus = 0.0;
            if r_vec == 1 { bonus += 0.05; }
            if r_vec == 2 || r_vec == 3 { bonus += 0.02; }
            if r_bm25 == 1 { bonus += 0.05; }
            if r_bm25 == 2 || r_bm25 == 3 { bonus += 0.02; }
            if r_fuzzy == 1 { bonus += 0.05; }
            if r_fuzzy == 2 || r_fuzzy == 3 { bonus += 0.02; }
    ...
    fn test_search_mode_parsing() {
        ...
        let modes = vec![
            "hybrid", "semantic", "keyword", "fuzzy", "bm25", "hybrid-rrf"
        ];
