...
async fn load_or_download() -> Result<NliModel> {
    let cache_dir = voidm_models::model_dir(NLI_MODEL_ID);
    ...
    let files = &[
        voidm_models::registry::NliModel::ONNX_FILE,
        voidm_models::registry::NliModel::TOKENIZER_FILE,
    ];
    
    // Check cache and download if missing
    if !voidm_models::is_model_cached(NLI_MODEL_ID, files) {
        voidm_models::download_model_files(
            NLI_MODEL_ID,
            files,
            voidm_models::registry::NliModel::SIZE_HINT,
            &cache_dir,
        ).await?;
    }
...
pub fn nli_cache_dir() -> PathBuf {
    voidm_embeddings::embedding_cache_dir()
        .parent()
        .map(|p| p.join("nli"))
        .unwrap_or_else(|| voidm_embeddings::embedding_cache_dir().join("nli"))
}
...
mod tests {
    ...
    // NOTE: test_model_inputs disabled - ort v2 RC doesn't expose inputs() method
    // Can be re-enabled once ort v2.0.0 final released with public API
    /*
    #[tokio::test]
