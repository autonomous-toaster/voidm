...
async fn load_or_download() -> Result<NerModel> {
    let cache_dir = voidm_models::model_dir(NER_MODEL_ID);
    ...
    let files = &[
        voidm_models::registry::NerModel::ONNX_FILE,
        voidm_models::registry::NerModel::TOKENIZER_FILE,
    ];

    if !voidm_models::is_model_cached(NER_MODEL_ID, files) {
        voidm_models::download_model_files(
            NER_MODEL_ID,
            files,
            voidm_models::registry::NerModel::SIZE_HINT,
            &cache_dir,
        ).await?;
    }
...
pub fn extract_entities(text: &str) -> Result<Vec<NamedEntity>> {
    ...
    // Post-processing: Correct entity types for common misclassifications
    // BERT sometimes classifies organizations as LOC. Fix by looking for org keywords.
    let org_keywords = ["university", "hospital", "institute", "institute", "center", "clinic", "school", "college", "corporation", "company", "inc", "ltd", "llc"];
    
    for entity in &mut entities {
        if entity.entity_type == "LOC" {
            let lower_text = entity.text.to_lowercase();
            if org_keywords.iter().any(|keyword| lower_text.contains(keyword)) {
                entity.entity_type = "ORG".to_string();
            }
        }
    }

    // Post-processing: Detect and extract technical entities (BERT returns zero or wrong entities for these)
    // Falls back to simple pattern detection for common technical patterns
    // This handles environment variables, config keys, API endpoints, etc.
    
    // Check if this looks like technical text (has many ALL_CAPS, slashes, version operators)
    let is_technical_text = {
        let all_caps_words: Vec<&str> = text.split_whitespace()
            .filter(|w| w.len() > 2 && w.chars().all(|c| c.is_uppercase() || c == '_' || !c.is_alphabetic()))
            .collect();
        let has_version_operators = text.contains(">=") || text.contains("==") || text.contains("~=") || text.contains("!=") || text.contains(">");
        let has_paths = text.contains("/api") || text.contains("_PROXY") || text.contains("://");
        let has_package_markers = text.contains("requires") || text.contains("import") || text.contains("setup.py");
        
        // Technical if: 2+ ALL_CAPS words OR version operators/paths/packages
        (all_caps_words.len() >= 2) || (has_version_operators || has_paths || has_package_markers)
    };
    
    if entities.is_empty() || is_technical_text {
        // Text is either empty or looks like technical documentation
        // Try to extract technical terms
        let technical_entities = extract_technical_entities(text);
        
        if is_technical_text && !technical_entities.is_empty() {
            // For pure technical text, prefer technical extraction over BERT
            entities = technical_entities;
        } else if entities.is_empty() {
            // Empty entities, use fallback
...
pub fn ner_model_downloaded() -> bool {
    let dir = voidm_models::model_dir(NER_MODEL_ID);
...
// ─── Technical entity extraction (fallback for zero-entity texts) ────────────

/// Extract technical entities (environment variables, constants, API endpoints)
/// Used when BERT returns zero entities (common for non-NER text like tech docs)
fn extract_technical_entities(text: &str) -> Vec<NamedEntity> {
    let mut entities = Vec::new();
    
    // Pattern 1: Technical identifiers - multiple patterns
    // 1a: ALL_CAPS_WITH_UNDERSCORES (HTTP_PROXY, LOG_LEVEL, CONTAINER_NAME)
    // 1b: lowercase_with_underscores (db_host, db_port)
    // 1c: Single ALLCAPS words in technical context (DEBUG, INFO, ERROR, etc.)
    // 1d: Other ALL_CAPS words (more lenient, captures HTTPS, HTTP, etc.)
    ...
    for word in &words {
        ...
        if let Some(idx) = text[char_pos..].find(word) {
            let start = char_pos + idx;
            ...
            let is_constant = (
                ...
                || (word.len() > 2
                    && word.chars().all(|c| c.is_lowercase() || c == '_' || c.is_numeric())
                    && word.contains('_')
            ...
            let is_false_positive = [
                "API", "URL", "ID", "IP", "XML", "JSON", "CSV", "HTTP", "HTTPS", "REST",
                "GET", "POST", "PUT", "DELETE", "PATCH", "SERVER", "CLIENT", "USER",
                "THE", "AND", "BUT", "FOR", "WITH", "THAT", "THIS", "WHICH", "FROM", "HAVE",
                "BEEN", "WERE", "WILL", "WOULD", "COULD", "SHOULD", "MUST", "WHAT", "WHEN",
            ].contains(word);
            
            if is_constant && !is_false_positive {
                // Higher confidence (0.85) for well-known patterns with underscores
                let confidence = if word.contains('_') { 0.85 } else { 0.75 };
                entities.push(NamedEntity {
                    text: word.to_string(),
    ...
    // Pattern 2: Path-like patterns (slashes indicate API endpoints or slash-separated constants)
    // Examples: /api/users, /api/users/{id}, HTTP_PROXY/HTTPS_PROXY, etc.
    let mut i = 0;
    ...
    while i < text_bytes.len() {
        if text_bytes[i] == b'/' {
            ...
            // Consume alphanumeric, digits, slashes, underscores, braces, hyphens
            while end < text_bytes.len() {
                let c = text_bytes[end] as char;
            ...
            if end > start + 1 {
                let path = std::str::from_utf8(&text_bytes[start..end]).unwrap_or("");
                if !path.ends_with('/') && path.len() > 2 {
                    // Check if this looks like slash-separated constants or API path
                    let looks_like_constants = path.contains('_') && path.contains('/');
                    let looks_like_api_path = !looks_like_constants && path.starts_with("/api");
                    
                    if looks_like_constants || looks_like_api_path {
                        entities.push(NamedEntity {
                            text: path.to_string(),
                            entity_type: "MISC".to_string(),
                            score: if looks_like_api_path { 0.70 } else { 0.80 },
                            start,
                            end,
                        });
                    }
    ...
    // Pattern 3: Common package names (lowercase words before version specifiers)
    // Examples: numpy>=1.19.0, scipy~=1.5.0, tensorflow==2.10.0
    let mut i = 0;
    let text_bytes = text.as_bytes();
    
    while i < text_bytes.len() {
        // Check if this looks like start of a word (after non-alphanumeric)
        let is_word_start = i == 0 || !text_bytes[i - 1].is_ascii_alphanumeric();
        
        if is_word_start && (text_bytes[i].is_ascii_lowercase() || text_bytes[i].is_ascii_digit()) {
            let word_start = i;
            let mut j = i;
            
            // Consume lowercase letters, digits, hyphens, underscores
            while j < text_bytes.len() && (text_bytes[j].is_ascii_lowercase() || text_bytes[j].is_ascii_digit() || text_bytes[j] == b'_' || text_bytes[j] == b'-') {
                j += 1;
            }
            
            if j > word_start && j - word_start >= 3 && j - word_start <= 20 {
                // Check if followed by version specifier: >=, ==, ~=, !=, or space then number
                let followed_by_version = j + 1 < text_bytes.len() && (
                    (text_bytes[j] == b'>' && text_bytes[j + 1] == b'=') ||
                    (text_bytes[j] == b'=' && text_bytes[j + 1] == b'=') ||
                    (text_bytes[j] == b'~' && text_bytes[j + 1] == b'=') ||
                    (text_bytes[j] == b'!' && text_bytes[j + 1] == b'=') ||
                    (text_bytes[j] == b'>' && j + 1 < text_bytes.len() && text_bytes[j + 1].is_ascii_digit()) ||
                    (text_bytes[j] == b'<' && j + 1 < text_bytes.len() && text_bytes[j + 1].is_ascii_digit())
                );
                
                if followed_by_version {
                    let package_name = std::str::from_utf8(&text_bytes[word_start..j]).unwrap_or("");
                    if !package_name.is_empty() && package_name != "setup" && package_name != "requires" && package_name != "package" {
                        entities.push(NamedEntity {
                            text: package_name.to_string(),
                            entity_type: "MISC".to_string(),
                            score: 0.80,
                            start: word_start,
                            end: j,
    ...
    // Deduplicate and handle overlaps: if two entities overlap, keep the longer one (higher confidence)
    entities.sort_by(|a, b| {
        // Sort by: score DESC (higher confidence first), then by length DESC (longer spans preferred)
    ...
    for entity in entities {
        ...
        let overlaps = result.iter().any(|kept: &NamedEntity| {
            // Overlaps if: (entity.start < kept.end) AND (entity.end > kept.start)
            entity.start < kept.end && entity.end > kept.start
        });
