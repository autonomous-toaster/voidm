//! Quality scoring and rating system for memories in voidm.
//!
//! Provides scoring and rating mechanisms to evaluate memory quality based on:
//! - Genericity (reusability across contexts)
//! - Abstraction (principle vs instance)
//! - Temporal independence (no time markers)
//! - Task independence (not tied to TODOs)
//! - Content substance (sufficient length)
...
/// Fallback when NER is not available - returns neutral score
#[cfg(not(feature = "ner"))]
fn entity_specificity_score(_content: &str) -> f32 {
    0.95  // Neutral score when NER model is not loaded
}
...
pub fn compute_quality_score(
    ...
    // But be lenient: experiential memories (episodic) naturally use "I" and "we"
    ...
    // Penalty: reduce from 0.15 per pronoun to 0.10 per pronoun
    // Allows memories like "I attended..." or "We discovered..." with minimal penalty
    let genericity = (1.0 - (personal_count as f32 * 0.10).min(1.0)).max(0.0);
    ...
    let abstraction = if has_personal_action { 0.3 } else { 0.95 };
    ...
    // Penalty: 0.88 - temporal framing is almost legitimate for episodic memories
    // Only slightly penalize to encourage generalization of insights beyond time context
    let temporal_independence = if has_temporal { 0.90 } else { 0.95 };
    ...
    if has_task_language {
        match memory_type {
            ...
            // Episodic naturally includes task language; allow it but with light penalty
            MemoryType::Episodic => {
                task_language_penalty = 0.05;
            }
            // Semantic, Contextual should not use task language
            _ => {
                task_language_penalty = 0.12;
            }
        }
    }

    // 6. Content substance: prefer 50+ words
    // Allow very short but focused memories (10+ words), scale gracefully
    let substance = if word_count < 10 {
        0.0
    } else if word_count < 50 {
        // Graceful scaling: starts at 0.50 for 10 words, reaches 0.95 at 50
        0.50 + (word_count as f32 - 10.0) / 40.0 * 0.45
    ...
    let score = (genericity * 0.19
        + abstraction * 0.20
        + temporal_independence * 0.25
        + task_independence * 0.15
        + substance * 0.20
        + entity_specificity * 0.06) - task_language_penalty;

    // Type-aware boosts: recognize inherent value of different memory types
    // Episodic: valuable for learning from experience
    // Semantic: captures principles with high reusability
    // Conceptual: abstract ideas have lasting value
    // Procedural: actionable how-to knowledge is immediately useful
    // Contextual: situational wisdom guides decision-making
    let final_score = match memory_type {
        MemoryType::Episodic => (score + 0.035).min(1.0),
        MemoryType::Semantic => (score + 0.03).min(1.0),
        MemoryType::Conceptual => (score + 0.015).min(1.0),
        MemoryType::Procedural => (score + 0.015).min(1.0),
        MemoryType::Contextual => (score + 0.015).min(1.0),
    };
