...
impl CrossEncoderReranker {
    ...
    pub fn score(&self, query: &str, document: &str) -> Result<f32> {
        ...
        let logit = {
            ...
            // Determine if model expects token_type_ids (based on model name, not runtime check)
            // Note: ORT v2 RC doesn't expose inputs() method, so we use model metadata instead
            let has_token_type_ids = model_has_token_type_ids(&self.model_name);
...
/// Check if a model expects token_type_ids input (based on model name, not runtime check)
/// MS-MARCO models use token_type_ids; others don't
fn model_has_token_type_ids(model_name: &str) -> bool {
    model_name.contains("ms-marco") || model_name.contains("mmarco")
}
