...
pub async fn run(_args: StatsArgs, pool: &SqlitePool, config: &Config, json: bool) -> Result<()> {
    ...
    if json {
        ...
        let edge_map: serde_json::Map<String, serde_json::Value> = edge_by_type.iter()
        ...
        if vec_count < total {
            println!("  Embedded:  {}/{} ({:.0}%)",
                vec_count, total, vec_count as f64 / total.max(1) as f64 * 100.0);
        } else if total > 0 {
            println!("  Embedded:  {}/{} (100%)", vec_count, total);
        }
        println!("Scopes:    {}", scope_count);
        if !top_tags.is_empty() {
            let tag_str: Vec<String> = top_tags.iter()
                .map(|(t, c)| format!("{}({})", t, c))
                .collect();
